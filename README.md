...
Adobe has never formally fixed this. The most reliable workaround at the Illustrator stage is to rasterize objects with `Object > Rasterize` before exporting, so no rasterization happens at export time. (However, this method isn’t 100% reliable. I’ve personally encountered cases where white lines still appeared when I do rasterization prior to exporting, and in Illustrator, white lines appeared immediately after clicking “Rasterize.” So please don’t skip the visual inspection!) This script handles the cases that slip through anyway.
...
Adobe 从未正式修复此问题。在 Illustrator 端最可靠的预防方式是导出前先用 `对象 > 栅格化` 手动栅格化，这样导出时就不会再发生二次栅格化。（但也不是百分百可靠! 本人曾遭遇过Rasterize后仍然出现白线/在AI（滚滚长江东逝水版）里点Rasterize后当场出现白线，请不要放弃肉眼查验！）
本脚本用于处理漏网之鱼。
